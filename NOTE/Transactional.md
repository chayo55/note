...
3. **Islostion**　　 　==隔离性==　事务与事务之间是相互隔离的
	- 隔离级别：
		...
		- `serializable`  
		串行，顾名思义，使并行的事务串行化，同一时间只能执行一个事务。一般是不会使用的，他会给每一行读取的数据加锁，会导致大量超时和[[锁]]竞争的问题。
...
==*Islostion*==. 　隔离性由隔离级别来保证。在MySQL中通过[[Transactional#MVCC Multi-Version Concurrency Control 多版本并发控制|MVCC]]来保证不可重复读和幻读的问题。避免了设置***串行化 serializable***，造成阻塞，并发性下降。

==*Durability*==. 　持久性由内存+redo log来保证，mysql修改数据同时在内存和redo log记录这次操作，事务提交的时候通过redo log刷盘，宕机的时候可以从redo log恢复

>redo log 的主要作用是用于数据库的崩溃恢复。redo log可以简单分为两部分，一是内存中的redo log buffer，二是磁盘中的 redo log file。写入redo log的时机为，先修改数据，后写日志，但redo 日志比数据先写回磁盘。以此来保证了数据库事务的持久性。
>undo log 主要记录的是数据的逻辑变化，为了在发生错误时回滚之前的操作，需要将之前的操作都记录下来，然后再发生错误时才可以回滚。
...
事务的并发控制，分为乐观（optimistic）并发控制和悲观（pressimistic）并发控制，以及（MVCC）多版本并发控制。
- **悲观并发控制**：即为对资源加锁。在这基础上，为了能尽量提高并发能力，设计了`共享锁`和`互斥锁`。当一个事务获得共享锁之后，它只可以进行读操作，所以`共享锁`也叫`读锁`；而当一个事务获得一行数据的互斥锁时，就可以对该行数据进行读和写操作，所以`互斥锁`也叫`写锁`。
- **乐观并发控制**：乐观锁并不是一种真正的锁，是一种并发控制的思想。有两种实现机制，`基于时间戳的协议`和`基于验证的协议`。
- **多版本并发控制**：前两种都是通过延迟或者终止相应的事务来解决事务之间的竞争条件（Race condition）来保证事务的可串行化。虽然这两种确实能够从根本上解决并发事务的可串行化的问题，但在实际环境中事务大都是只读的，读请求是写请求的很多倍，如果写请求和读请求之前没有并发控制机制，那么最坏的情况也是读请求读到了已经写入的数据，这对很多应用完全是可以接受的。所以引入了另一种并发控制机制 - （MVCC）多版本并发控制。
